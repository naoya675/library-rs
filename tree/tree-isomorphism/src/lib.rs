...
pub fn hash_d(tree: &[Vec<usize>], root: usize, r: &[ModintMersenne61]) -> Vec<ModintMersenne61> {
    let n = tree.len();
    let mut hash = vec![ModintMersenne61::new(1); n];
    let mut depth = vec![0; n];
    let mut stack = vec![(root, None, false)];
    while let Some((v, p, visited)) = stack.pop() {
        if !visited {
            stack.push((v, p, true));
            hash[v] = ModintMersenne61::new(1);
            for &to in &tree[v] {
                if Some(to) != p {
                    depth[to] = depth[v] + 1;
...
pub fn hash_h(tree: &[Vec<usize>], root: usize, r: &[ModintMersenne61]) -> Vec<ModintMersenne61> {
    let n = tree.len();
    let mut hash = vec![ModintMersenne61::new(1); n];
